...
pub fn copy_bits<S: PrimitiveType, D: PrimitiveType>(
    src: &[S],
    src_offset: usize,
    dst: &mut [D],
    dst_offset: usize,
...
pub unsafe fn copy_bits_raw<S: PrimitiveType, D: PrimitiveType>(
    src: *const S,
    src_offset: usize,
    dst: *mut D,
    dst_offset: usize,
...
fn validate_range<P: PrimitiveType>(data: &[P], offset: usize, bit_count: usize) -> bool {
    let slice_bit_count = data.len() * P::BIT_COUNT;
    let bit_range_end = offset + bit_count;
    bit_range_end <= slice_bit_count
}

struct Source<P: PrimitiveType> {
    ptr: *const P,
    buffer: CountedBits<P>,
}

impl<P: PrimitiveType> Source<P> {
...
struct Destination<P: PrimitiveType> {
    ptr: *mut P,
    offset: usize,
    buffer: CountedBits<P>,
}

impl<P: PrimitiveType> Destination<P> {
    ...
    fn write_next_elem_keeping_surrounding_bits(&mut self, bits: CountedBits<P>) {
        let msb_keep_bit_count = P::BIT_COUNT - bits.count - self.offset;
        let lsb_keep_bit_count = self.offset;

        let mut elem_bits = unsafe { self.ptr.read() };
        elem_bits &= make_bits_pattern([msb_keep_bit_count, bits.count, lsb_keep_bit_count]);
...
impl<P: PrimitiveType> From<P> for CountedBits<P> {
    fn from(bits: P) -> Self {
...
impl<P: PrimitiveType> UpperHex for CountedBits<P> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        self.fmt(f, 'X')
    }
}

impl<P: PrimitiveType> LowerHex for CountedBits<P> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        self.fmt(f, 'x')
    }
}

impl<P: PrimitiveType> Binary for CountedBits<P> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        self.fmt(f, 'b')
    }
}
