...
use crate::{
    BitAccessor, BitStr, BitValue, BitsPrimitive, BitsPrimitiveDiscriminant, BitsPrimitiveSelector,
    PrimitiveAccessor,
};
...
impl<'a> Iter<'a> {
    ...
    #[inline]
    pub fn next_primitive<P: BitsPrimitive>(&mut self) -> Option<P> {
        self.next_at_front(P::BIT_COUNT, IterPrimitiveSelector::select_output)
    }

    #[inline]
    pub fn next_primitive_back<P: BitsPrimitive>(&mut self) -> Option<P> {
        self.next_at_back(P::BIT_COUNT, IterPrimitiveSelector::select_output)
    }
...
impl BitsPrimitiveSelector for IterBitSelector {
    ...
    fn select<U: BitsPrimitive>(self) -> Self::Output {
        let (ptr, offset) = unsafe { normalize_ptr_and_offset(self.ptr.cast::<U>(), self.offset) };
