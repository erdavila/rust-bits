...
fn accessing() {
    let index: usize;
    let bit: Option<BitValue> = bit_str.at(index);
    ...
    let bit: Option<&mut Primitive<u8>> = bit_str.primitive_at_mut::<u8>(index); // ... and other primitive types
